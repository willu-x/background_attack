...
def load_Non_IID_clean_data(number_of_total_participants, dirichlet_alpha, num_workers=0):
    ...
    # raw data(or built in data set) -> data set -> data loader
    train_set = datasets.CIFAR10(
        root='/home/will/backup/data',
        train=True,
        download=True,
...
def train_cv(target_model, local_model, epoch, criterion, sampled_participants, lr_init=0.001, traget_lr=0.2, train_loaders=None):
    ...
    for participant_id in sampled_participants:
        ...
        weight_difference, difference_flat = get_weight_difference(
            global_model_copy, model.named_parameters())
        clipped_weight_difference, l2_norm = clip_grad(
            s_norm, weight_difference, difference_flat)
        weight_difference, difference_flat = get_weight_difference(
            global_model_copy, clipped_weight_difference)
        copy_params(model, weight_difference)  # ？？？将模型参数更新为差值
        l2_norm_after_clip, l2_norm_np = get_l2_norm(
            global_model_copy, model.named_parameters())
...
def Generate_trigger(model, mask_grad_list):
    ...
    for i in range(2000):
        ...
        scheduler.step()
        if i % 200 == 0:
            print('Gen trigger loss', loss)
            # print('Gen trigger lr',scheduler.get_last_lr())
    return trigger
...
# def Generate_trigger1(model, mask_grad_list):
#     #根据model和mask_grad_list生成trigger
#     #return:trigger
...
#         scheduler.step()
#         if i % 200 == 0:
#             print('Gen trigger1 loss', loss)
#             # print('Gen trigger1 lr',scheduler.get_last_lr())
#     return trigger

# def Generate_trigger2(model, mask_grad_list):
#     #根据model和mask_grad_list生成trigger
#     #return:trigger
...
#         scheduler.step()
#         if i % 200 == 0:
#             print('Gen trigger2 loss', loss)
#             # print('Gen trigger1 lr',scheduler.get_last_lr())
#     return trigger


# def Generate_trigger3(model, mask_grad_list):
    #根据model和mask_grad_list生成trigger
    #return:trigger
...
def generate_trigger(nameOfTrigger, model, mask_grad_list, locationX, locationY):
    ...
    def set_optimization_mask(x_start, y_start):
        optimization_mask = torch.zeros(
            1, 3, graph_width, graph_width, dtype=torch.bool).cuda()
        optimization_mask[:, :, x_start:x_start +
                          trigger_width, y_start:y_start+trigger_height] = True
...
def poison_train(model, trainloaders, criterion, epoch, trigger):
    #对model进行训练
    #model:模型
    #trainloader:训练集
    #optimizer:优化器
...
def poison_train_trigger1(model, trainloaders, criterion, epoch, trigger1, mask_grad_list=None):
    #对model进行训练
    #model:模型
    #trainloader:训练集
    #optimizer:优化器
    ...
    # scheduler = lr_scheduler.StepLR(poison_optimizer, step_size=100, gamma=0.1)
    model.train()
    trigger1 = torch.reshape(trigger1, (3, 3, 3))
    for i in range(epoch):
        # 对多个trainloader中的数据进行训练
        for trainloader in trainloaders:
            for batch_idx, (data, target) in enumerate(trainloader):
                ...
                target[index] = 0
                poison_data = data[index]
                poison_data[:, :, 29:32, 26:29] = trigger1
                data[index] = poison_data
                poison_optimizer.zero_grad()
                output = model(data)
                loss = criterion(output, target)
...
def poison_train_trigger2(model, trainloaders, criterion, epoch, trigger2, mask_grad_list=None):
    #对model进行训练
    #model:模型
    #trainloader:训练集
    #optimizer:优化器
    ...
    # scheduler = lr_scheduler.StepLR(poison_optimizer, step_size=100, gamma=0.1)
    model.train()
    trigger2 = torch.reshape(trigger2, (3, 3, 3))
    for i in range(epoch):
        # 对多个trainloader中的数据进行训练
        for trainloader in trainloaders:
            for batch_idx, (data, target) in enumerate(trainloader):
                ...
                target[index] = 0
                poison_data = data[index]
                poison_data[:, :, 26:29, 26:29] = trigger2
                data[index] = poison_data
                poison_optimizer.zero_grad()
                output = model(data)
                loss = criterion(output, target)
...
def poison_train_trigger3(model, trainloaders, criterion, epoch, trigger3, mask_grad_list=None):
    #对model进行训练
    #model:模型
    #trainloader:训练集
    #optimizer:优化器
    ...
    # scheduler = lr_scheduler.StepLR(poison_optimizer, step_size=100, gamma=0.1)
    model.train()
    trigger3 = torch.reshape(trigger3, (3, 3, 3))
    for i in range(epoch):
        # 对多个trainloader中的数据进行训练
        for trainloader in trainloaders:
            for batch_idx, (data, target) in enumerate(trainloader):
                ...
                target[index] = 0
                poison_data = data[index]
                poison_data[:, :, 26:29, 29:32] = trigger3
                data[index] = poison_data
                poison_optimizer.zero_grad()
                output = model(data)
                loss = criterion(output, target)
...
def test_poison_model_with_globol_trigger_four_locals(model, testloader, globol_trigger):
    #测试模型
    model.eval()
    correct = 0
    with torch.no_grad():
    ...
    with torch.no_grad():
        for data, target in testloader:
            ...
            target = torch.zeros_like(index[0])
            poison_data = data[index]
            poison_data[:, :, 26:32, 26:32] = globol_trigger
            data = poison_data
            output = model(data)
            # get the index of the max log-probability
            pred = output.argmax(dim=1, keepdim=True)
